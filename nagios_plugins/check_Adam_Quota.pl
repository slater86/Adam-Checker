...
my $Quota=$data->{Customer}{Account}{ByteQuota};
foreach my $TotalUsage (@{$data->{Customer}->{Account}->{Usage}->{Bucket}}) {
	if ( $TotalUsage->{desc} =~ /Download/ )
	{
        $Usage="$TotalUsage->{Usage}->{content}";
	}
}

print "OK - $Usage of $Quota used |Download=$Usage; Total=$Quota;";
